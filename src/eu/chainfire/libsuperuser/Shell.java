...
public class Shell {
	 ...
	 * output is still stored in a List<String>, and as such doing something
	 * like "ls -lR /" will probably have you run out of memory.
	 * 
	 * @param shell
	...
	public static List<String> run(String shell, String[] commands,
		...
		try {
			...
			StreamGobbler STDOUT = new StreamGobbler(shellUpper + "-",
					process.getInputStream(), res);
			StreamGobbler STDERR = new StreamGobbler(shellUpper + "*",
					process.getErrorStream(), wantSTDERR ? res : null);
	...
	public static class SH {
		...
		public static List<String> run(List<String> commands) {
			return Shell.run("sh",
					commands.toArray(new String[commands.size()]), false);
		}
	...
	public static class SU {
		...
		public static boolean available() {
			...
			List<String> ret = SU.run(new String[] { "id", "echo -EOC-" });
			if (ret == null) {
				return false;
			}
		 ...
		 * 
		 * @param internal
		 *            Request human-readable version or application internal
		 *            version
